...
import { type JSONValue, OverlayMergeError } from "../types";
import {
	captureWarnings,
	createOrdOverlay,
	createOverlayPatch,
...
test("appends text to selected string fields", async () => {
	...
	const merged = applyOverlayToDocument(openApiSource, overlay) as Record<
		string,
		unknown
	>;
	const info = merged.info as Record<string, unknown>;
	assert.equal(
		info.description,
		"This is just a sample API Additional overlay details.",
	);
});
