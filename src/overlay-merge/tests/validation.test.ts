...
test("validateOverlayInput reports schema errors for invalid overlay documents", () => {
  ...
  assert.ok(result.errors.some((issue) => issue.message.includes('Missing required property "ordOverlay"')));
  assert.ok(result.errors.some((issue) => issue.path === "$.patches" && issue.message.includes("must NOT have fewer than 1 items")));
...
test("validateOverlaySemantics emits SHOULD warnings for ambiguous operation patches", () => {
  ...
  assert.ok(result.warnings.some((issue) => issue.message.includes("target.definitionType is RECOMMENDED")));
  assert.ok(result.warnings.some((issue) => issue.message.includes("target.ordId alone can be ambiguous")));
  assert.ok(result.warnings.some((issue) => issue.message.includes('resolver falls back to OpenAPI -> MCP -> A2A detection order')));
  assert.ok(result.warnings.some((issue) => issue.message.includes("describedSystemVersion SHOULD be provided")));
  assert.ok(result.warnings.some((issue) => issue.message.includes("describedSystemType SHOULD also be provided")));
});
...
test("validateOverlaySemantics does not emit metadata-definition warnings for ordId-only patches", () => {
  ...
  assert.equal(result.errors.length, 0);
  assert.equal(result.warnings.some((issue) => issue.message.includes("target.definitionType is RECOMMENDED")), false);
});
...
test("validateOverlaySemantics reports unsupported selector and patch data errors", () => {
  ...
  assert.ok(result.errors.some((issue) => issue.message.includes('Patch action "append" requires string data.')));
  assert.ok(result.errors.some((issue) => issue.message.includes('The "operation" selector is not supported for definitionType "asyncapi-v2"')));
});
...
test("validateOverlaySemantics validates deprecated definitionType values and JSONPath syntax", () => {
  ...
  assert.ok(result.errors.some((issue) => issue.message.includes('MUST NOT use the deprecated literal "custom"')));
  assert.ok(result.errors.some((issue) => issue.message.includes("Invalid JSONPath expression")));
});
...
test("validateOverlaySemantics warns about ambiguous propertyType selectors and errors because OData support is not implemented", () => {
  ...
  assert.ok(result.warnings.some((issue) => issue.message.includes("propertyType selectors may need selector.entityType")));
  assert.ok(result.errors.some((issue) => issue.message.includes('The "propertyType" selector is not supported yet.')));
});
...
test("validateTargetDocumentForDefinitionType detects target shape mismatches", () => {
  ...
  assert.ok(openApi31Issues.some((issue) => issue.message.includes('does not match definitionType "openapi-v3.1+"')));
