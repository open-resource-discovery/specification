...
const KNOWN_NON_JSON_OR_YAML_DEFINITION_TYPES = new Set(["edmx", "graphql-sdl", "wsdl-v1", "wsdl-v2"]);
const ajv = new Ajv({
  allErrors: true,
...
const validateOverlaySchemaWithAjv = ajv.compile(overlaySchema as Record<string, unknown>);
...
export function validateOverlaySchema(input: unknown): OverlayValidationIssue[] {
  if (validateOverlaySchemaWithAjv(input)) {
    return [];
  }

  return (validateOverlaySchemaWithAjv.errors ?? []).map((error) =>
    createIssue("error", toDisplayPath(error.instancePath), formatAjvError(error)),
  );
}
...
export function validateOverlaySemantics(
  ...
  const hasMetadataDefinitionSelectors = overlay.patches.some((patch) => getSelectorKind(patch.selector) !== "ordId");
  if (
  ...
  overlay.patches.forEach((patch, patchIndex) => {
    ...
    validateSelectorSemantics(selectorKind, patch.selector, patchPath, definitionType, errors, warnings);
  });
...
export function validateTargetDocumentForDefinitionType(
  ...
  if (definitionType === "openapi-v2") {
    return validateExactVersionPrefix(targetDocument.swagger, "2.", "$.swagger", "openapi-v2");
  }
  ...
  if (definitionType === "a2a-agent-card") {
    return Array.isArray(targetDocument.skills)
      ? []
      : [createIssue("error", "$.skills", 'A2A Agent Card targets must contain a top-level "skills" array.')];
  }

  if (definitionType === "csdl-json") {
    return typeof targetDocument.$Version === "string"
      ? []
      : [createIssue("error", "$.$Version", 'CSDL JSON targets must contain a top-level "$Version" string.')];
  }

  if (definitionType === "asyncapi-v2") {
    return validateExactVersionPrefix(targetDocument.asyncapi, "2.", "$.asyncapi", "asyncapi-v2");
  }

  if (definitionType === "ord:overlay:v1") {
    return targetDocument.ordOverlay === "0.1"
      ? []
      : [createIssue("error", "$.ordOverlay", 'ORD Overlay targets must contain ordOverlay: "0.1".')];
  }

  if (isSpecificationId(definitionType) && overlay?.patches.some((patch) => getSelectorKind(patch.selector) === "operation")) {
    return Array.isArray(targetDocument.tools)
      ? []
      : [createIssue("error", "$.tools", 'Specification ID targets using the "operation" selector must expose a top-level "tools" array.')];
  }
...
export function throwOnOverlayValidationErrors(errors: OverlayValidationIssue[]): void {
  if (errors.length === 0) {
    return;
  }
...
export function formatOverlayValidationErrors(errors: OverlayValidationIssue[]): string {
  return `Overlay validation failed:\n${errors.map((error) => `- ${error.path}: ${error.message}`).join("\n")}`;
}

function addPerspectiveWarnings(overlay: ORDOverlay, warnings: OverlayValidationIssue[]): void {
  if (overlay.perspective === "system-type" && overlay.describedSystemType === undefined) {
    warnings.push(
      createIssue(
        "warning",
        "$.describedSystemType",
  ...
  if (overlay.perspective === "system-instance" && overlay.describedSystemInstance === undefined) {
    warnings.push(
      createIssue(
        "warning",
...
function validatePatchData(
  ...
  if ((patch.action === "update" || patch.action === "merge") && patch.data === undefined) {
    errors.push(createIssue("error", `${patchPath}.data`, `Patch action "${patch.action}" requires data.`));
  }

  if (patch.action === "append" && typeof patch.data !== "string") {
    errors.push(createIssue("error", `${patchPath}.data`, 'Patch action "append" requires string data.'));
  }
}
...
function validateSelectorSemantics(
  ...
  if (selectorKind === "jsonPath") {
    ...
    if (definitionType !== undefined && KNOWN_NON_JSON_OR_YAML_DEFINITION_TYPES.has(definitionType)) {
      errors.push(
        createIssue(
          "error",
          selectorPath,
  ...
  if (selectorKind === "entityType" || selectorKind === "propertyType") {
    ...
    if (definitionType !== undefined && definitionType !== "edmx" && definitionType !== "csdl-json") {
      errors.push(
        createIssue(
          "error",
          selectorPath,
    ...
    errors.push(createIssue("error", selectorPath, `The "${selectorKind}" selector is not supported yet. ${detail}`));
  }
}

function validateJsonPathExpression(expression: string, path: string, errors: OverlayValidationIssue[]): void {
  try {
    jsonpath.nodes({}, expression);
  } catch (error: unknown) {
    const reason = error instanceof Error ? error.message : String(error);
    errors.push(createIssue("error", path, `Invalid JSONPath expression: ${reason}`));
  }
}

function validateOpenApiVersion(value: unknown, definitionType: "openapi-v3" | "openapi-v3.1+"): OverlayValidationIssue[] {
  if (typeof value !== "string") {
    return [createIssue("error", "$.openapi", `OpenAPI targets for "${definitionType}" must contain a top-level "openapi" version string.`)];
  }

  const match = /^(\d+)\.(\d+)(?:[.].*)?$/.exec(value);
  if (match === null) {
    return [createIssue("error", "$.openapi", `OpenAPI version "${value}" is not in a recognised semantic version format.`)];
  }
...
function validateExactVersionPrefix(
  ...
  return typeof value === "string" && value.startsWith(expectedPrefix)
    ? []
    : [createIssue("error", path, `Target document does not match definitionType "${definitionType}".`)];
}

function resolveDefinitionType(overlay: ORDOverlay, context: OverlayMergeContext | undefined): string | undefined {
  if (typeof context?.definitionType === "string") {
    return context.definitionType;
  }

  return typeof overlay.target?.definitionType === "string" ? overlay.target.definitionType : undefined;
}

function supportsOperationSelector(definitionType: string): boolean {
  return isOpenApiDefinitionType(definitionType) || definitionType === "a2a-agent-card" || isSpecificationId(definitionType);
}

function isOpenApiDefinitionType(definitionType: string): boolean {
  return definitionType === "openapi-v2" || definitionType === "openapi-v3" || definitionType === "openapi-v3.1+";
}
...
type SelectorKind = "jsonPath" | "ordId" | "operation" | "entityType" | "propertyType" | "unknown";

function getSelectorKind(selector: ORDOverlay["patches"][number]["selector"]): SelectorKind {
  ...
  if (isJSONObject(selector) && typeof selector.entityType === "string" && !("propertyType" in selector)) {
    return "entityType";
  }
...
function createIssue(level: "error" | "warning", path: string, message: string): OverlayValidationIssue {
