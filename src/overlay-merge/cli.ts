...
async function readJsonFile(path: string): Promise<JSONValue> {
	...
	if (!isJsonValue(parsed)) {
		throw new OverlayMergeError(
			`File ${path} does not contain valid JSON values.`,
		);
	}
...
function assertJsonCliPath(
	path: string,
	label: "Overlay" | "Input" | "Output",
): void {
...
function printHelp(): void {
	...
	process.stderr.write(
		`  node dist/overlay-merge/cli.js --overlay <overlay.json> --input <target.json> [options]\n\n`,
	);
	...
	process.stderr.write(
		`                                 Note: the CLI currently supports JSON files only; YAML is not yet implemented\n`,
	);
