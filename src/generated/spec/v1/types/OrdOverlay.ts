 ...
 * - `entityType`: entity type level - targets an OData EntityType by name (not the EntitySet in the container)
 * - `propertyType`: property type level - targets a property on an OData EntityType or ComplexType
 ...
 *   - `entityType` and `propertyType`: OData (`edmx` for v2/v4, `csdl-json` for v4). `entityType` targets the EntityType definition, not the EntitySet in the container.
...
export interface OverlaySelectorByEntityType {
   ...
   *
   * For OData, this targets the EntityType definition (the type declared in the schema),
   * not the EntitySet in the EntityContainer.
   * Use the unqualified name (e.g. `Customer`) if unique within the document,
   * or the namespace-qualified name (e.g. `OData.Demo.Customer`) to be explicit.
   *
   * Note: EntitySet-level patching (e.g. for Capability annotations on a specific entity set)
   * is not supported by this selector. Use `jsonPath` as a fallback in those cases.
   */
  entityType: string;
}
export interface OverlaySelectorByPropertyType {
   ...
   * - `csdl-json` (OData v4 CSDL JSON)
   *
   * For OData, this targets a Property (or NavigationProperty) on an EntityType or ComplexType.
   * Use the unqualified property name (e.g. `BirthDate`).
   * If the property name is unique across all types in the document, this field alone is sufficient.
   * If it is ambiguous (same name on multiple types), provide `entityType` to disambiguate.
   */
  propertyType: string;
  /**
   ...
   * For OData, this identifies the containing EntityType or ComplexType by name.
   * Use the unqualified name (e.g. `Customer`) or namespace-qualified name (e.g. `OData.Demo.Customer`).
   */
  entityType?: string;
}
