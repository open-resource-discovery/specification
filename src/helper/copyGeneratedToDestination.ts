...
async function generateAggregatorPushApi(): Promise<void> {
  const [documentSchemaJson, source] = await Promise.all([
    readFile(documentSchemaPath, "utf8"),
    readFile(aggregatorPushApiSourcePath, "utf8"),
  ]);
  const documentSchema: unknown = JSON.parse(documentSchemaJson);
