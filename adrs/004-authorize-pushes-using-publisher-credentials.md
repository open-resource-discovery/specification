...
A request can contain described-system identifiers and ORD namespaces, but those values cannot establish who may publish them. At the same time, an extra publication-scope identifier duplicates context already present in credentials and ORD documents. How should an aggregator authorize pushes without requiring one credential technology or a transport-specific scope ID?

## Decision Drivers

- Keep the document endpoint free of context parameters.
...
- **Optional v1 choices:** aggregators choose an established machine-to-machine authentication mechanism and can issue several credentials for one publisher.
