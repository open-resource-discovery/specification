...
This is often an [authority namespace](../index.md#authority-namespace), but can also be a reused system namespace when another system type owns the shared definition.
...
The Group Type could even be defined globally. If the Group Type is shared across different applications, it should use the namespace of the owner that governs the taxonomy. This is commonly an [authority namespace](../index.md#authority-namespace), but it can also be a system namespace when one system type owns the taxonomy definition.
