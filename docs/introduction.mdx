...
<div className="img-box" style={{aspectRatio: "3869/1690"}}>
...
<div className="img-box" style={{aspectRatio: "3716/1714"}}>
...
Open Resource Discovery (ORD) is a protocol that **enables applications and services to self-describe their exposed resources and capabilities**.
...
An application publishes ORD information through a standardized transport.
It can expose a read-only entry point for pull-based discovery or push its metadata to an ORD aggregator.

<div className="img-box" style={{"maxWidth": "480px", aspectRatio: "1771/1422"}}>
...
But ORD can also be used to describe higher-level concepts like [Entity Types](./spec-v1/interfaces/Document.md#entity-type) (Business Objects) and [Data Products](./spec-v1/interfaces/Document.md#data-product).
...
<div className="img-box" style={{aspectRatio: "2757/1652"}}>
...
- **Customer System Landscape**: Describes a real system landscape as it actually exists, e.g. for a customer ("as-is view").
  - SAP uses the [Unified Customer Landscape](#unified-customer-landscape) (UCL) as central aggregator, that will discover and combine ORD information and re-expose it as a (customer specific) system landscape metadata view.
...
- Execute an HTTP GET request on the [.well-known/open-resource-discovery](https://ord-reference-application.cfapps.sap.hana.ondemand.com/.well-known/open-resource-discovery) entry point.
  - With a successful response we now know that ORD is supported, where to find the actual information (`documents.url`) and how to access it (`documents.accessStrategies`)
- Execute an HTTP GET request on the [first ORD document](https://ord-reference-application.cfapps.sap.hana.ondemand.com/open-resource-discovery/v1/documents/system-version), we discovered from the previous step.
  - Here we now find the actual information, like the APIs and Events the application exposes, but also taxonomy like information like what product it is or how the information is structured into packages.
...
<div className="img-box" style={{aspectRatio: "2265/1290"}}>
...
In the UI screenshot, the most notable concept is the [ORD System Instance](./spec-v1/index.md#system-instance), which is usually a tenant.
If ORD information is provided, the detail view can show them and they will be made available to other systems in the same formation or account context.
...
ORD as a protocol standardizes both the **information model** and the [behavior](./spec-v1/index.md#ord-behavior) how the information is exchanged.
...
<div className="img-box" style={{aspectRatio: "3180/1390"}}>
...
by also standardizing how the information is discovered, transported and accessed. See [ORD Transport Modes](./spec-v1/index.md#ord-transport-modes) for an example.

<div className="img-box" style={{aspectRatio: "3941/1370"}}>
...
<div className="img-box" style={{aspectRatio: "3979/1214"}}>
...
They must implement at least one standardized ORD transport mode.
With pull, they expose an ORD Provider API; with push, they submit ORD Documents and resource definitions to an ORD aggregator.
