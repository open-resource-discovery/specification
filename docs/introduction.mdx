...
In the UI screenshot, the most notable concept is the [ORD System Instance](./spec-v1/index.md#def-system-instance), which is usually a tenant.
...
ORD as a protocol standardizes both the **information model** and the [behavior](./spec-v1/index.md#def-ord-behavior) how the information are exchanged.
...
The [ORD Provider](./spec-v1/index.md#def-ord-provider) is a system that describes itself via ORD, so it's metadata becomes available to interested ORD Consumers.
...
The [ORD Aggregator](./spec-v1/index.md#def-ord-aggregator) has the job to simplify the life of ORD Consumers.
...
The [ORD Consumer](./spec-v1/index.md#def-ord-consumer) is interested in learning about the resources and capabilities of other systems.
