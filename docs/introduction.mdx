...
An application publishes ORD information through a standardized transport. It can expose a read-only entry point for pull-based discovery or push its metadata to an ORD aggregator.
...
With pull, they expose an ORD Provider API; with push, they send ORD documents and resource definitions to an ORD aggregator.
