...
- Clarified ORD Overlay patch semantics: unmatched `merge` and `update` MUST error while unmatched `remove` is an idempotent no-op; `jsonPath` applies to every match; missing removal-mask entries are ignored; and omitted-data root removal MUST error while root removal masks remain valid.
